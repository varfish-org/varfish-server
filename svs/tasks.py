"""Models and supporting code for running celery tasks"""
...
@app.task(bind=True)
def run_sv_query_bg_job(_self, sv_filter_bg_job_pk):
    """Task to execute a ``svs.tasks.FilterBgJob``."""
    return models.run_sv_query_bg_job(pk=sv_filter_bg_job_pk)
...
@app.task(bind=True)
def build_bg_sv_set_task(_self, *, chromosomes=None):
    ...
    bg_db.build_bg_sv_set(spec_bg_job, chromosomes=chromosomes)
