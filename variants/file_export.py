...
class CaseExporterBase:
    ...
    def _is_gm_enabled(self):
        ...
        return (
            settings.VARFISH_ENABLE_GESTALT_MATCHER and self.query_args.get("gm_enabled")
        )

    def _is_pedia_enabled(self):
        """Return whether PEDIA prioritization is enabled in this query."""
        return (
            settings.VARFISH_ENABLE_PEDIA and self.query_args.get("pedia_enabled")
        )
    ...
    def _yield_smallvars(self):
        ...
        with contextlib.closing(self.query.run(self.query_args)) as result:
            ...
            for i, small_var in enumerate(_result):
                if self._is_prioritization_enabled() or self._is_pathogenicity_enabled() or self._is_gm_enabled or \
                    self._is_pedia_enabled():
                    if i % steps == 0:
        ...
        if self._is_pedia_enabled():
            try:
                ...
                for line in result:
                    ...
                    if all((line.entrez_id, hasattr(line, 'phenotype_score'), hasattr(line, 'pathogenicity_score'),
                        ...
                        payload["gestalt_score"] = 0 if line.gm_score == float("inf") else line.gm_score
                ...
                return {
                    str(gene_id): score
                        for gene_id, _, score in get_pedia_scores(scores)
                }
            except ConnectionError as e:
                self.job.add_log_entry(e)
        else:
