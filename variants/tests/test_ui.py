...
class TestVariantsCaseFilterView(TestUIBase):
    ...
    def test_variant_filter_case_multi_bookmark_one_variant(self):
        ...
        selectors[0].click()
        WebDriverWait(self.selenium, self.wait_time).until(selectors[0].is_selected)
        ...
        # un-select variant 1
        selectors[0].click()
        WebDriverWait(self.selenium, self.wait_time).until(lambda: not selectors[0].is_selected())
        WebDriverWait(self.selenium, self.wait_time).until(selectors[1].is_selected)

        # select variant 3
        selectors[2].click()
        WebDriverWait(self.selenium, self.wait_time).until(selectors[2].is_selected)
    ...
    def test_preset_whole_exome_on_effect_settings(self):
        ...
        # verify correctness
        WebDriverWait(self.selenium, self.wait_time).until(
            self.selenium.find_element_by_id("id_effect_group_all").is_selected
        )
