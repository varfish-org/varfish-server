...
class TestSmallVariantQueryListCreateAjaxView(TestProjectAPIPermissionBase):
    def setUp(self):
        super().setUp()
...
class TestSmallVariantQueryRetrieveUpdateDestroyAjaxView(TestProjectAPIPermissionBase):
    def setUp(self):
        super().setUp()
        self.case = CaseFactory(project=self.project)
    ...
    def test_get(self):
        url = reverse(
            "variants:ajax-query-retrieve-update-destroy", kwargs={"smallvariantquery": self.query.sodar_uuid}
        )
        good_users = [
            self.superuser,
            self.owner_as.user,
...
class TestSmallVariantQueryResultSetListAjaxView(TestProjectAPIPermissionBase):
    def setUp(self):
        super().setUp()
        self.case = CaseFactory(project=self.project)
    ...
    def test_get(self):
        url = reverse("variants:ajax-query-result-set-list", kwargs={"smallvariantquery": self.query.sodar_uuid})
        good_users = [
            self.superuser,
            self.contributor_as.user,
            self.owner_as.user,
...
class TestSmallVariantQueryResultSetRetrieveAjaxView(TestProjectAPIPermissionBase):
    def setUp(self):
        super().setUp()
        self.case = CaseFactory(project=self.project)
        self.smallvariantqueryresultset = SmallVariantQueryResultSetFactory(smallvariantquery__case=self.case)
...
class TestSmallVariantQueryResultRowListAjaxView(TestProjectAPIPermissionBase):
    def setUp(self):
        super().setUp()
        self.case = CaseFactory(project=self.project)
        self.smallvariantqueryresultset = SmallVariantQueryResultSetFactory(smallvariantquery__case=self.case)
