...
class PedigreeViewSet(mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    """Allow retrieval only of ``Pedigree`` record for a given case."""
    ...
    def list(self, request, *args, **kwargs):
        ...
        with transaction.atomic():
            if not CaseAnalysis.objects.filter(case=case).exists():
                CaseAnalysis.objects.create(case=case, name="default")
...
class CaseAnalysisSessionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        case = get_object_or_404(Case.objects.all(), sodar_uuid=self.kwargs["case"])
        with transaction.atomic():
            caseanalysis = CaseAnalysis.objects.get_or_create(case=case, defaults={"name": "default"})
