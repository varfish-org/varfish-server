...
export const actions = {
  ...
  createNewSubmissionSet({ state, commit }) {
    const titles = Object.values(state.submissionSets).map(
  ...
  async wizardSave({ state, commit }) {
    ...
    async function _wizardSaveSubmissionSet(
      { state, commit },
      submissionSetExists
    ) {
      if (submissionSetExists) {
        ...
        commit(
          ...
          state.submissionSetList
            ...
            .map((ss) =>
              ss.sodar_uuid === state.currentSubmissionSet.sodar_uuid
                ? apiSet
                : ss
            )
        )
    ...
    async function _wizardSaveApplySubmittingOrgs({ state, commit }, apiSet) {
      ...
      for (
        let i = 0;
        i < state.currentSubmissionSet.submitting_orgs.length;
        i++
      ) {
        ...
        if (localUuid in state.oldModel.submittingOrgs) {
          ...
          commit('ADD_SUBMITTING_ORG_TO_SUBMISSION_SET', {
      ...
      if (apiSet.sodar_uuid in state.oldModel.submissionSets) {
        const oldSubmissionSet =
          state.oldModel.submissionSets[apiSet.sodar_uuid]
    ...
    async function _wizardSaveSubmission(
      { state, commit },
      apiSet,
      sortOrder,
      localSubmissionUuid
      ...
      if (submissionExists) {
        const oldSubmission =
          state.oldModel.submissions[localSubmission.sodar_uuid]
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        const localSubmissionIndividualExists =
          localSI.sodar_uuid in state.oldModel.submissionIndividuals
    ...
    const apiSet = await _wizardSaveSubmissionSet(
      { state, commit },
      submissionSetExists
    )
    ...
    if (submissionSetExists) {
      const oldSubmissionSet =
        state.oldModel.submissionSets[state.currentSubmissionSet.sodar_uuid]
  ...
  async wizardRemove({ state, commit }) {
    ...
    for (const submissionUuid of submissionUuids) {
      const submissionInvidualUuids = Array.from(
        state.submissions[submissionUuid].submission_individuals
      )
   ...
   */
  updateCurrentSubmissionSetOrganisations({ commit }, organisations) {
  ...
  createSubmissionInCurrentSubmissionSet(
    { commit },
    { smallVariant, submission, individualUuids }
  ) {
    commit('CREATE_SUBMISSION_IN_CURRENT_SUBMISSION_SET', {
      smallVariant,
      submission,
      individualUuids,
    })
  ...
  moveCurrentSubmission({ state, commit }, up) {
    ...
    const prev = (acc, loc) =>
      loc.sort_order < c && loc.sort_order > acc.sort_order ? loc : acc
    const other = state.currentSubmissionSet.submissions
  ...
  deleteCurrentSubmission({ state, commit }) {
    const submissionInvidualUuids = Array.from(
      state.currentSubmission.submission_individuals
    )
    for (const submissionInvidualUuid of submissionInvidualUuids) {
      commit('DELETE_SUBMISSION_INDIVIDUAL', submissionInvidualUuid)
    }
  ...
  moveSubmissionIndividual({ state, commit }, { submissionIndividual, up }) {
    ...
    const prev = (acc, loc) =>
      loc.sort_order < c && loc.sort_order > acc.sort_order ? loc : acc
    const other = state.currentSubmission.submission_individuals
  ...
  removeSubmissionIndividualFromCurrentSubmission(
    ...
    commit('UPDATE_CURRENT_SUBMISSION', {
      key: 'submission_individuals',
      value: state.currentSubmission.submission_individuals.filter(
 ...
 */
export function extractVariantZygosity(smallVariant, individualUuids, state) {
...
export const mutations = {
  ...
  SET_CURRENT_SUBMISSION_SET(state, submissionSetUuid) {
    ...
    if (submissionSetUuid === null) {
      Vue.set(state, 'currentSubmissionSet', null)
    } else {
      Vue.set(
        state,
        'currentSubmissionSet',
        state.submissionSets[submissionSetUuid]
      )
    }
  },
  ...
  INITIALIZE_SUBMISSION_SET_ORGANISATIONS(state) {
    for (const submissionSet of Object.values(state.submissionSets)) {
      submissionSet.organisations = submissionSet.submitting_orgs.map(
        (subOrgUuid) => state.submittingOrgs[subOrgUuid].organisation
      )
    }
  },
  ...
  SAVE_OLD_MODEL(state) {
    Vue.set(
      ...
      'oldModel',
      JSON.parse(
        JSON.stringify(Object.fromEntries(MODEL_KEYS.map((k) => [k, state[k]])))
      )
    )
  },
  ...
  UPDATE_CURRENT_SUBMISSION_SET(state, { key, value }) {
    ...
    console.assert(
      key in state.currentSubmissionSet,
      state.currentSubmissionSet,
      key
    )
  ...
  UPDATE_CURRENT_SUBMISSION_SET_ORGANISATIONS(state, shouldUuids) {
    ...
    const removeSOUuids = new Set(
      state.currentSubmissionSet.submitting_orgs.filter((soUuid) =>
        removeUuids.has(state.submittingOrgs[soUuid].organisation)
      )
    )
    ...
    Vue.set(
      state.currentSubmissionSet,
      'submitting_orgs',
      state.currentSubmissionSet.submitting_orgs.filter(
    ...
    for (const orgUuid of addUuids) {
      ...
      state.currentSubmissionSet.submitting_orgs.push(
        newSubmittingOrg.sodar_uuid
      )
    }
    ...
    for (const soUuid of state.currentSubmissionSet.submitting_orgs) {
      state.submittingOrgs[soUuid].sort_order =
        orgUuidToSortOrder[state.submittingOrgs[soUuid].organisation]
    }
    state.currentSubmissionSet.submitting_orgs.sort((a, b) => {
      return (
        state.submittingOrgs[a].sort_order - state.submittingOrgs[b].sort_order
      )
    })
  },
  ...
  UPDATE_CURRENT_SUBMISSION(state, { key, value }) {
    ...
    console.assert(
      key.slice(0, 1) === '_' || key in state.currentSubmission,
      state.currentSubmission,
      key
    )
  ...
  UPDATE_SUBMISSION(state, { submission, key, value }) {
    console.assert(
      submission.sodar_uuid in state.submissions,
      state.submissions,
      submission.sodar_uuid
    )
    console.assert(
      key in state.submissions[submission.sodar_uuid],
      state.submissions,
      submission.sodar_uuid
    )
  ...
  DELETE_SUBMISSION_SET(state, submissionSetUuid) {
    if (
      state.currentSubmissionSet &&
      state.currentSubmissionSet.sodar_uuid === submissionSetUuid
    ) {
    ...
    Vue.set(
      state,
      'submissionSetList',
      state.submissionSetList.filter(
        (ss) => ss.sodar_uuid !== submissionSetUuid
      )
    )
  },

  CREATE_SUBMISSION_IN_CURRENT_SUBMISSION_SET(
    state,
    { smallVariant, submission, individualUuids }
  ) {
  ...
  ADD_INDIVIDUAL_TO_CURRENT_SUBMISSION(state, { individual, smallVariants }) {
    ...
    Vue.set(state.submissionIndividuals, newUuid, {
      sodar_uuid: newUuid,
      sort_order: Object.keys(state.currentSubmission.submission_individuals)
        .length,
      individual: individual.sodar_uuid,
      submission: state.currentSubmission.sodar_uuid,
      phenotypes: JSON.parse(
        JSON.stringify(state.individuals[individual.sodar_uuid].phenotype_terms)
      ),
      variant_zygosity: variantZygosity,
      variant_allele_count: variantAlleleCount,
      variant_origin: 'germline',
  ...
  ADD_SUBMISSION_INDIVIDUAL_TO_SUBMISSION(
    state,
    { submissionIndividual, submission }
  ) {
    state.submissions[submission].submission_individuals.push(
      submissionIndividual
    )
  },
}
