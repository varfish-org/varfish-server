class QueryBuilder:
    ...
    def build_comphet_query(self, kwargs):
        ...
        return (
            ...
            FROM ({query}) sv
            LEFT OUTER JOIN main_dbsnp d USING (release, chromosome, position, reference, alternative)
            LEFT OUTER JOIN main_hgnc h {hgnc}
            WHERE
                (p1_c > 0) AND
                (p2_c > 0);
    ...
    def build_comphet_query_part(self, gt_father, gt_mother, kwargs):
        ...
        return (
            ...
            FROM main_smallvariant AS sv
            LEFT OUTER JOIN main_case c ON (sv.case_id = c.id)
            WHERE (
