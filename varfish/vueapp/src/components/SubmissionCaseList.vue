<template>
  <div class="border-top">
    ...
    <div v-for="item in caseSubmissionIndividuals" :key="item.sodar_uuid">
      <submission-case-list-entry v-model="item.wrapped"></submission-case-list-entry>
    </div>
    <p v-if="caseSubmissionIndividuals.length === 0" class="text-center text-secondary font-italic">
      No individuals have been added to this submission yet.
    </p>

    <b-modal id="modal-add-case" scrollable title="Add Case to Submission" hide-footer>
      <p>
        Select an individual to add for this variant.
      </p>
      ...
      <p class="mb-0">
        You can only add each individual once.
      </p>
    </b-modal>
  </div>
</template>
...
export default {
  ...
  computed: {
     ...
     *
     * We must return them in a wrapper object as we we cannot iterate and use it as models directory,
     * cf. https://stackoverflow.com/q/57974480
     */
    caseSubmissionIndividuals () {
      const result = this.currentSubmission.submission_individuals
        .map(uuid => ({ wrapped: this.submissionIndividuals[uuid] }))
  ...
  methods: {
    ...mapActions('clinvarExport', [
      'addIndividualToCurrentSubmission'
    ]),
    ...
    getModalIndividualList () {
      const blockedIndividualUuids = new Set(
        this.currentSubmission.submission_individuals
          .map(uuid => this.submissionIndividuals[uuid].individual)
      )
      const result = Object.values(this.individuals)
        .filter(obj => !blockedIndividualUuids.has(obj.sodar_uuid))
      return result
    },
    /**
    ...
    getPhenotypeDisplay (individual) {
      return (individual.phenotype_terms || []).map(t => `(${t.term_id}) ${t.term_name}`).join(', ')
    }
  }
}
</script>
