 ...
 */
async function apiFetch ({ baseUrl, csrfToken }, { entity, uuid }, method, payload) {
...
async function apiDelete (entity, uuid, appContext) {
  ...
  const response = await apiFetch(appContext, { entity: entityToken, uuid: uuid }, 'DELETE')
  if (!response.ok) {
...
async function getEntities (entity, appContext) {
  const response = await apiFetch(appContext, { entity: entity.replace('_', '') })
  return await response.json()
}
...
export default {
  ...
  async updateSubmissionSet (submissionSet, appContext) {
    return await apiUpdate('submission_set', submissionSet.sodar_uuid, submissionSet, appContext)
  },

  async deleteSubmissionSet (submissionSet, appContext) {
    return await apiDelete('submission_set', submissionSet.sodar_uuid, appContext)
  },
  ...
  async updateSubmission (submission, appContext) {
    return await apiUpdate('submission', submission.sodar_uuid, submission, appContext)
  },
  ...
  async createSubmissionIndividual (submissionIndividual, appContext) {
    return await apiCreate('submission_individual', submissionIndividual, appContext)
  },

  async updateSubmissionIndividual (submissionIndividual, appContext) {
    return await apiUpdate('submission_individual', submissionIndividual.sodar_uuid, submissionIndividual, appContext)
  },

  async deleteSubmissionIndividual (submissionIndividual, appContext) {
    return await apiDelete('submission_individual', submissionIndividual.sodar_uuid, appContext)
  },
  ...
  async updateSubmittingOrg (submittingOrg, appContext) {
    return await apiUpdate('submitting_org', submittingOrg.sodar_uuid, submittingOrg, appContext)
  },
  ...
  async genericTermQuery ({ baseUrl, csrfToken }, name, label, termId) {
    const response = await fetch(`${baseUrl}/query-${name}/?query=${encodeURIComponent(termId)}`, {
      method: 'GET',
      credentials: 'same-origin',
      headers: {
        Accept: 'application/json',
  ...
  async queryOmim (appContext, termId) {
    return await this.genericTermQuery(appContext, 'omim', 'OMIM', termId)
  },

  async queryHpo (appContext, termId) {
    return await this.genericTermQuery(appContext, 'hpo', 'HPO', termId)
  }
}
